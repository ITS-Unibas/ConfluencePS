...
- Function ```Remove'ConfluenceLabel```: Function used to invoke ```Get-Label``` instead of ```Get-ConfluenceLabel```

### Changed

- Reverted "Introduced 'Uni' prefix [v3.0.1]" for genericity
...
- Function ```Get-ConfluencePageProperty```: Improved error handling
